...
fn main()
{
    ...
    let actor_io_client = enter!{tokio_runtime, || { ItWorksMacTaskActorState::spawn() }};

    actor_io_client.input_sender().send(WorkJob::DoesItWork).expect("Error: Sender Error");

    let res = actor_io_client.output_receiver_lock().expect("Error: Output Receiver Lock Error").blocking_recv().expect("Error: Receiver Error");

    println!("{}\n", res);

    println!("ItWorksTaskActor");

    let actor_io_client = enter!{tokio_runtime, || { ItWorksTaskActorState::spawn() }};
